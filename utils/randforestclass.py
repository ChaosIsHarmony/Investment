...
def generate_dataset(data, limit, offset, data_aug_per_sample=0, shuffle=False):
	'''
...
X_train, y_train, X_test, y_test = get_datasets("bitcoin", 500)
...
base_preds = [1 for _ in range(len(y_train))]
count = 0
for i, x in enumerate(y_train):
	if x == base_preds[i]:
		count += 1

print(f"Baseline accuracy by choosing mode: {count/len(y_train)}")
...
def save_confusion_matrix_heatmap(y_test, preds):
	...
	#class_names = ["BUY 3X", "BUY 2X", "BUY X", "HODL", "SELL Y", "SELL 2Y", "SELL 3Y"]
